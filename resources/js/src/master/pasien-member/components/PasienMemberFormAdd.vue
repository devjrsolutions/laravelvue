<template>
    <div>
        <v-card>
            <v-card-title>
                <v-icon>assignment</v-icon> Create Pasien Member
            </v-card-title>
            <v-divider></v-divider>
...
<script>
    export default {
        ...
        mounted() {
            console.log('components.PasienMemberFormAdd.vue');
        ...
        methods: {
            save() {
                ...
                axios.post('/system/pasien-member',payload).then(function(response) {
                    ...
                    self.$eventBus.$emit('PASIENMEMBER_ADDED');
