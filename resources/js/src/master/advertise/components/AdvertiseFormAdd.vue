<template>
    <div>
        <v-card>
            <v-card-title>
                <v-icon>assignment</v-icon> Create Advertise
            </v-card-title>
            <v-divider></v-divider>
...
<script>
    export default {
        ...
        mounted() {
            // console.log('components.AdvertiseFormAdd.vue');
        ...
        methods: {
            save() {
                ...
                axios.post('/system/advertise',payload).then(function(response) {
                    ...
                    self.$eventBus.$emit('ADVERTISE_ADDED');
