<template>
    <div>
        <v-card>
            <v-card-title>
                <v-icon>assignment</v-icon> Create Pasien Status
            </v-card-title>
            <v-divider></v-divider>
...
<script>
    export default {
        ...
        mounted() {
            console.log('components.PasienStatusFormAdd.vue');
        ...
        methods: {
            save() {
                ...
                axios.post('/system/pasien-status',payload).then(function(response) {
                    ...
                    self.$eventBus.$emit('PASIENSTATUS_ADDED');
