<template>
    <div>
        <v-card>
            <v-card-title>
                <v-icon>assignment</v-icon> Create Gender
            </v-card-title>
            <v-divider></v-divider>
...
<script>
    export default {
        ...
        mounted() {
            console.log('components.GenderFormAdd.vue');
        ...
        methods: {
            save() {
                ...
                axios.post('/system/gender',payload).then(function(response) {
                    ...
                    self.$eventBus.$emit('GENDER_ADDED');
