        ...
        <v-card class="pt-3">
            <div class="d-flex flex-row">
                <div class="flex-grow-1 pa-2">
                    <v-btn @click="$router.push({name:'pasien.create'})" class="primary lighten-1" dark>
                        Tambah Pasien
            ...
            <template v-slot:header="{props:{headers}}">
                ...
                <tr>
                    <th v-for="header in headers">
                        ...
                        <span v-if="header.value=='klinik_code'"><v-icon>mdi-person</v-icon> {{header.text}}</span>
                        ...
                        <span v-if="header.value=='create_date'"><v-icon>mdi-person</v-icon> {{header.text}}</span>
                        <span v-if="header.value=='confirm_date'"><v-icon>mdi-person</v-icon> {{header.text}}</span> -->
                    ...
                    <tr v-for="item in items" :key="item.id">
                        <td>
                            <div class="text-center">
                                <v-btn @click="$router.push({name:'pasien.edit',params:{id: item.id}})" class="ma-2" outlined fab small color="info">
                        ...
                        <td>{{ $appFormatters.formatDate(item.create_date) }}</td>
                        <td>{{ $appFormatters.formatDate(item.confirm_date) }}</td>
...
<script>
    export default {
        data () {
            return {
                headers: [
                    ...
                    { text: 'Klinik Code', value: 'klinik_code', align: 'left', sortable: false },
                    ...
                    { text: 'Create Date', value: 'create_date', align: 'left', sortable: false },
                    { text: 'Confirm Date', value: 'confirm_date', align: 'left', sortable: false },
        ...
        mounted() {
            ...
            self.$eventBus.$on(['PASIEN_ADDED','PASIEN_UPDATED','PASIEN_DELETED','GROUP_ADDED'],()=>{
                self.loadPasien(()=>{});
            });
        ...
        methods: {
            trash(pasien) {
                ...
                self.$store.commit('showDialog',{
                    ...
                    message: "Are you sure you want to delete this pasien?",
                    okCb: ()=>{

                        axios.delete('/system/pasien/' + pasien.id).then(function(response) {
                            ...
                            self.$eventBus.$emit('PASIEN_DELETED');
            ...
            loadPasien(cb) {
                ...
                axios.get('/system/pasien',{params: params}).then(function(response) {
